...
public class DisplayManager{
	...
	public DisplayManager(double radius, double horizontalScaling,
			double verticalScaling, double angle, double centerX, double centerY,
			AbstractBoard board){
		this.radius = radius;
	 ...
	 */
	private Point2D.Double _boardToScreen(final double x, final double y, double z){
	 ...
	 * @return The screen coordinates
	 */
	private Point2D.Double _boardToScreen(final double x, final double y, final double z, final double theta){
		double cos = Math.cos(theta);
		double sin = Math.sin(theta);
		Point2D.Double point = _boardToScreen(x, y, z);
		return new Point2D.Double(point.x * cos - sin * point.y, point.x * sin + cos * point.y);
	}
	
	/**
	 * Gives the screen coordinates given the board coordinates, the tilt angle of the board and the scaling factors
	 * NOTE: The origin is (0,0)
	 * @param x
	 * @param y
	 * @param z
	 ...
	 * @return The screen coordinates
	 */
	private Point2D.Double _boardToScreen(final double x, final double y, final double z, final double theta, final double horizontalScale, final double verticalScale){
		Point2D.Double point = _boardToScreen(x, y, z, theta);
	...
	public Point2D.Double boardToScreen(final double x, final double y, final double z){
		Point2D.Double point = _boardToScreen(x, y, z, angle, scale.x, scale.y);
	...
	private double[] _screenToBoard(final double u, final double v){
		return new double[]{2.0*v/(3.0*size.y)+u/size.x,2.0*v/(3.0*size.y)-u/size.x,-4*v/(3.0*size.y)};
	}
	
	/**
	 * Gives the screen coordinates given the board coordinates and the rotation angle
	 * NOTE: The origin is (0,0)
	 * @param u
	 * @param v
	...
	/**
	 * Gives the screen coordinates given the board coordinates, the rotation angle and the scale factors
	 * @param u
	 * @param v
	 * @param theta The rotation angle applied to the board
	 * @param su
	 ...
	 * @return The screen coordinates
	 */
	private double[] _screenToBoard(final double u, final double v, final double theta, final double su, final double sv){
		return _screenToBoard(u / su, v / sv, theta);
	}

	/**
	 * Gives the board coordinates given the screen coordinates, the rotation angle, the scale factors and the screen origin
	 * @param u
	 * @param v
	 * @param theta The rotation angle applied to the board
	 * @param su
	 ...
	 * @return The screen coordinates
	 */
	public Coordinates screenToSquare(Point2D point){
	 ...
	 */
	public Coordinates nearestSquare(double x, double y, double z){
	...
	public Path2D.Double hexagon(final Coordinates h) {
		...
		for (int i = 0; i <= 6; i++) {
			double theta = i * Math.PI / 3.0 + Math.PI / 6.0 + angle;
			Point2D.Double point = new Point2D.Double(center.x + scale.x * Math.cos(theta) * radius, center.y + scale.y * Math.sin(theta) * radius);
			if (i == 0)
